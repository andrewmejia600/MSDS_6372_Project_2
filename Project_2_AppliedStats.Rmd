...
Senstvty = x[1]/(x[1] + x[2]) * 100 
Specivty = x[2,2]/(x[1,2] + x[2,2]) * 100 
...
pred = predict(main.logr_l_3, newdata = Bank_mrkt_data_partition_train, type="response")
...
categorical_variables_plot_list <-lapply(categorical_variables, function(x) catplot(Bank_mrkt_data %>% keep(is.factor) ,x, "y"))
# output plot grid
library(cowplot)
cowplot::plot_grid(plotlist = categorical_variables_plot_list)
...
# First option is to remove all unknown observations
...
# Second option is to selectively remove or convert unknows
...
There are huge number of unknowns (1731 observations). We can't delete these records as there are 251 subscribers for term deposit. Instead of deleting, it is better to convert to university.degree as the proportion is ~ to unknown. Lets convert to university.degree.

```{r}
Bank_mrkt_data = Bank_mrkt_data %>% mutate(education = dplyr::recode(education, "unknown" = "university.degree"))
...
```{r}
Bank_mrkt_data = Bank_mrkt_data %>% dplyr::select(-default)
```
...
Bank_mrkt_data = Bank_mrkt_data %>% mutate(loan = dplyr::recode(loan, "unknown" = "no"))
```
...
```{r}
Bank_mrkt_data = Bank_mrkt_data %>% mutate(pdays_dummy = if_else(pdays == 999, "0", "1")) %>% dplyr::select(-pdays) %>% dplyr::select(-previous)
```
